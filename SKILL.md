...
- If payload restoration fails in `attachBaseContext`, suppress the framework's pending business-provider list before returning and route from the shell Application to the protected failure surface. A missing encrypted business Provider must never replace the intended failure UI with `ClassNotFoundException`.
...
- Read every post-link-patched ELF seal/digest slot through a volatile accessor. At optimized native build levels, ordinary reads from source-initialized `const` zero slots may be constant-folded and ignore the digest patched into the final SO.
